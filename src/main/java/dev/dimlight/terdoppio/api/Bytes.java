...
/**
 * Bytes is an immutable wrapper around a byte array.
 * <p>
 * Bytes always copies on creation and never lets its internal byte array escape. Its modeled after Java's String which
 * is an immutable wrapper around a char array. It was created because there is nothing in Java like
 * it at the moment. Its very nice having this immutable type, as it avoids having to do defensive
 * copies to ensure correctness. Maybe one day Java will have equivalents of String, StringBuilder,
 * and Charsequence for bytes.
 *
 * <p>
 * The reason to not use ByteBuffer is that it's not immutable, even a read only ByteBuffer
...
public final class Bytes implements Comparable<Bytes>, Serializable {
    ...
    private static final Comparator<byte[]> BYTES_ARRAY_UNSIGNED_COMPARATOR = (a, b) -> {
        final int sharedLen = Math.min(a.length, b.length);

        for (int i = 0; i < sharedLen; ++i) {
            final int comparison = Integer.compare((int) a[i] & 0xFF, (int) b[i] & 0xFF);
            if (comparison != 0) {
                return comparison;
            }
        }
    ...
    public int compareTo(byte[] bytes, int offset, int len) {
        if (!(offset >= 0 && len >= 0 && offset + len <= bytes.length)) {
            throw new IllegalArgumentException("Invalid offsets/len");
        }
    ...
    private int compareToUnchecked(byte[] bytes, int offset, int len) {
        if (this.length == this.data.length && len == bytes.length) {
            return BYTES_ARRAY_UNSIGNED_COMPARATOR.compare(this.data, bytes);
        } else {
     ...
     */
    public static Bytes of(byte[] data, int offset, int length) {
     ...
     */
    public static Bytes of(CharSequence cs) {
     ...
     */
    public static Bytes of(String s, Charset c) {
        Objects.requireNonNull(s);
